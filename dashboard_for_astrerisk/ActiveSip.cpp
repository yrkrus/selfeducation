...
ACTIVE_SIP::Parsing::Parsing(const char *fileActiveSip)
{
	...
	if (sip.is_open())
	{
		// разберем  “”“ ѕќ ј в 1 потоке все происходит потом сделать многопочную!!!  и сразу в Ѕƒ!!
...
// получаем активный лист операторов которые у нас сейчас в линии наход€тс€
...
std::string ACTIVE_SIP::Parsing::findParsing(std::string str, ACTIVE_SIP::Currentfind find, const std::string number_operator)
{
	...
	//	std::cout << "lines.size = " << lines.size() << "\n";
		...
		case ACTIVE_SIP::Currentfind::talk_time_find:		// врем€ разговора
		{
			return lines[9];
			break;
		}
...
// парсинг нахождени€ активного sip оператора
...
void ACTIVE_SIP::Parsing::findActiveOperators(const char *fileOperators, std::string queue)
{
	...
	if (!file.is_open()) {
		// не получаетс€ открыть файл уходим из функции
		return;		
	}
	...
	{
		// заносить активные CAllers не требуетс€
